...
def _validate_args(args):
    ...
    if args.first_frame is None and "first_frame" in EXAMPLE_PROMPT[args.task]:
        args.first_frame = EXAMPLE_PROMPT[args.task]["first_frame"]
    if args.last_frame is None and "last_frame" in EXAMPLE_PROMPT[args.task]:
...
def _parse_args():
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Whether to use bidirectional sampling."
    )
...
def generate(args):
    ...
    if "t2v" in args.task:
        ...
        if args.bidirectional_sampling:
            logging.info("Using bidirectional sampling.")
            video = wan_flf2v.generate_bidirectional(
                args.prompt,
                first_frame,
                last_frame,
                max_area=MAX_AREA_CONFIGS[args.size],
            ...
            logging.info("Using unidirectional sampling.")
